...
module.exports = {
  ...
  async function getChat(req, res) {
    const chat = await Chat.findById(req.params.chatId).populate('users').populate('message').populate('message.user');
    res.json(chat);
  }
  ...
  async function findChat(req, res) {
    ...
    if (chatId === null) {
      ...
      const chat = await Chat.findById(chatId).populate("users").populate("message");
      res.json(chat);
    }
  }
